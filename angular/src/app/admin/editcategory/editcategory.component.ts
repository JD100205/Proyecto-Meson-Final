  ...
  fetchCategoryById(): void {
    ...
    if (this.categoryId) {
      this.apiService.getCategoryById(this.categoryId).subscribe({
        ...
        error: (error) => {
          console.log((error))
          this.message = error?.error?.message || "No se pudo obtener la categoría por su Id";
          setTimeout(() => {
            this.message = null;
          }, 3000)
        }
      ...
      this.apiService.updateCategory(this.categoryId, this.categoryForm.value).subscribe({
        next: (response) => {
          if (response.status === 200) {
            this.message = "Categoria actualizada correctamente"
            setTimeout(() => {
              this.message = null;
        ...
        error: (error) => {
          console.log(error)
          this.message = error?.error?.message || "no se pudo actualizar la categoria";
        }
      })
    }
  }
